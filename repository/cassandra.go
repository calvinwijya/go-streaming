...
func (c *cassandraRepo) Store(data []map[string]interface{}) (err error) {
	...
	for _, dataRow := range data {
		...
		for key, value := range dataRow {
			...
			bindValue = append(bindValue, "?")
		}

		wg.Add(1)
		go func() {
			defer wg.Done()
			queryStmp := `INSERT INTO ` + c.keyspace + `.` + tableName + ` (` + strings.Join(onlyColumn, ",") + `)` + ` VALUES (` + strings.Join(bindValue, ",") + `)` + `;`
			query := c.session.Query(queryStmp, args...)
			err := query.Exec()
			if err != nil {
				return
			}
